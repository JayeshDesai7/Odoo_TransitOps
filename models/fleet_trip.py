...
class FleetTrip(models.Model):
    """New model — justified exception to 'prefer inherit'.
    No existing Odoo model represents a dispatch/trip lifecycle; fleet.vehicle
    and res.partner are consumed (not duplicated) via FK.
    """
    _name = 'fleet.trip'
    ...
    name = fields.Char(
        default=lambda self: 'New', copy=False, readonly=True,
    )
    ...
    driver_id = fields.Many2one(
        'res.partner', string='Driver', required=True, tracking=True,
    ...
    def _check_driver_eligible(self, driver):
        ...
        if driver.license_expiry_date and driver.license_expiry_date < fields.Date.today():
            raise UserError(f'{driver.name}\'s license expired on '
    ...
    def _check_vehicle_eligible(self, vehicle):
        ...
        if state_name in ('Retired', 'In Shop'):
            raise UserError(f'{vehicle.display_name} is {state_name} and '
                             f'cannot be dispatched.')
